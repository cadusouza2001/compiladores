...
private void run() {
    try {
        ...
        ClassDeclaration ast = parse(lexer, symbolFactory);
        if (ast == null) return;
    ...
    private ClassDeclaration parse(FoolLexer lexer, SymbolFactory symbolFactory) {
        try {
            FoolParser parser = new FoolParser(lexer, symbolFactory);
            return (ClassDeclaration) parser.parse().value;
        } catch (Exception e) {
    ...
    private boolean analyzeSemantics(ClassDeclaration ast, SemanticAnalyzer semanticAnalyzer) {
    ...
    private void generateCode(ClassDeclaration ast, IntermediateCodeGenerator codeGenerator) {
